...
PRONGHORN = ClusterProfile(
    ...
    remote_base="/scratch/user/MOR",   # NOT ~ (reference_pronghorn_transfer)
)
...
PERLMUTTER = ClusterProfile(
    ...
    host="user@perlmutter.example.gov",
    remote_base=None,                # scratch path varies — pass an absolute remote_dir
)
...
def _resolve_remote(remote_dir, profile):
    ...
    if profile.remote_base:
        return profile.remote_base.rstrip("/") + "/" + remote_dir.strip("/")
    raise ValueError(
...
""" % (profile.host, profile.host, remote_path))
...
""" % (profile.host, dest, profile.host, remote_path, exc_rsync, exc_tar))
