...
def copy_back_script_text(remote_dir, profile=PRONGHORN, extra_excludes=(),
    ...
    NEVER OVERWRITES A NEWER LOCAL FILE (2026-09-23). Both branches skip a
    file whose local copy is newer than the cluster's: rsync `--update`; tar
    `--keep-newer-files` (in both GNU tar and bsdtar; they also keep a file
    of EQUAL age). Incident: `Foundations/dissociation_probe/pkg/STATUS.md`
    was rewritten locally an hour after ship.sh ran, and the I2-wave
    copy_back replaced it with the stale shipped copy.
    Why this cannot skip a real cluster output: the guard only acts on a file
    that exists on BOTH sides. A new output is absent locally and is always
    pulled. A file pulled before carries the cluster mtime (rsync -a and tar
    both restore it), so any later cluster write is strictly newer (rsync
    also re-sends an equal-mtime file whose size differs). An output is
    wrongly skipped only if its local copy got a later mtime some other way:
    a plain `cp` without -p, a restored Drive conflict copy, or a local clock
    far ahead of the cluster's. The remaining gap is an interrupted transfer,
    which leaves a truncated file stamped "now" that the guard would then
    protect forever (measured on openrsync: `--partial --update` kept 172 MB
    of a 3 GB file and exited 0). So:
      * rsync keeps partial files in `--partial-dir=.rsync-partial`, never
        under the real name, and deletes those dirs after a successful run
        (openrsync leaves them behind, and they would sync to Drive);
      * tar pulls into a `mktemp -d` staging dir outside the package, which
        lives inside Drive, and merges only a COMPLETE pull. Both tar streams
        use pax format, which keeps sub-second mtimes. With whole-second
        mtimes, a file the cluster changed in the same second as the last
        pull would count as "same age" and be skipped. Staging temporarily
        doubles the disk used by the pull.
    `-k`/`--skip-old-files` is NOT an acceptable tar fallback: it keeps every
    existing file, which would freeze a mid-campaign `.out` at its first
    pulled state.
...
# Never deletes; skips CP2K restart files (large/regenerable). Never overwrites
# a local file that is NEWER than the cluster copy (e.g. a doc edited after
# ship.sh). Prefers rsync; falls back to tar-over-ssh where rsync is absent
# (Git Bash).
set -euo pipefail
...
echo "into  $DEST"
if command -v rsync >/dev/null 2>&1; then
  # Partials go to .rsync-partial/, never under the real name: --update would
  # otherwise protect a truncated file forever.
  rsync -avz --update --partial-dir=.rsync-partial --timeout=300 \\
    -e 'ssh -o ServerAliveInterval=30 -o ServerAliveCountMax=10' \\
%s \\
    "$HOST:$REMOTE/" "$DEST"/
  find "$DEST" -type d -name .rsync-partial -prune -exec rm -rf {} +
else
  echo "(rsync not found; using tar over ssh — e.g. Git Bash on Windows)"
  # Stage OUTSIDE the package (it lives in Google Drive) and merge only a
  # COMPLETE pull, so an interrupted stream never leaves a truncated file that
  # --keep-newer-files would then protect.
  STAGE="$(mktemp -d)"
  trap 'rm -rf "$STAGE"' EXIT
  ssh "$HOST" "tar czf - -C '$REMOTE' --format=pax --warning=no-file-changed --ignore-failed-read \\
%s \\
    ." | tar xzf - -C "$STAGE"
  KEEP="--keep-newer-files"
  if tar --version 2>/dev/null | grep -q 'GNU tar'; then
    KEEP="$KEEP --warning=no-ignore-newer"   # else one line per unchanged file
  fi
  tar cf - --format=pax -C "$STAGE" . | tar xf - $KEEP -C "$DEST"
fi
