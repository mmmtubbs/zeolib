...
8. **zeolib is a git repo** (since 2026-08-25; scope = `zeolib/` only — the
   ...
   **The repo is PUBLIC**: no login accounts, host names, site paths, or
   credentials in committed code — cluster identity resolves from environment
   or `~/.config/zeolib/clusters.json` (selftest guards this). Results and
   protocol numbers in docs are a deliberate choice, not an accident.
9. **Every HPC job dir carries a viewable input geometry** (2026-09-22):
   coordinates reach a job dir ONLY through `fileio.write_coords_inc`, which
   also writes a 1-frame extended-XYZ beside the `.inc` (`coords.inc` →
   `coords.xyz`) for opening in OVITO/ASE/VMD. Pass `cell=` when writing from
   bare symbols/positions (an ASE Atoms brings its own cell), or the view
   has no box. Never `shutil.copy` a coords.inc into a new dir — re-write it.
...
|---|---|---|
| `constants.py` | Unit conversions (Ha→eV, Ha/Bohr→eV/Å, bar→eV/Å³), production protocol numbers (1500/100/1e-7/LBFGS), GTH basis+potential table (13 elements), guest multiplicities, **`ATOM_GROUND_MULTIPLICITY`** (2026-09-22: isolated pseudo-atom ground-state spin, Hund's rules without SOC, for atomic reference energies; parity-pinned against `VALENCE_ELECTRONS`), **`VALENCE_ELECTRONS` + `valence_electron_count`** (the GTH `-qN` suffixes, so the count can never drift from the pseudopotentials in use — CP2K's `MULTIPLICITY` defaults to "derive it", so a generator emitting RKS or bare `UKS` must assert an EVEN count rather than discover it in an abort; tests/rks_uks_parity), 16 MOR combos + `COMBOS_BY_FRAMEWORK`/`combo_names`/`n_al_for_ratio` (Foundations: + the 17 historical FAU combos, FAUn = Si:Al n, verified vs FullOpt_out compositions; charge balance selftest-pinned) | `tests/na_training_set/collect_train.py`, `pipeline_archive/stage1a_v1/cell-opt-example.inp`, `binding/setup_fullopt.py`; FAU map Foundations 2026-07-10 |
| `geometry.py` | MIC vector/dist/all-pairs, `unwrap`, `centroid_unwrapped` (the PBC-centroid bug fix), **`continuous_trajectory(positions, cells, groups)`** (2026-09-22: re-image a possibly multi-job, possibly variable-cell trajectory so every atom moves continuously, groups stay whole, and the last frame is wrapped — viewing optimisation histories without face-crossing jumps), min/mean pair distances, `wrap_to_cell`, `cell_matrix`, `perp_widths`. Cell always explicit, in TWO dispatch forms (Foundations 2026-07-10): (3,) box lengths = the orthorhombic fast path (behaviour unchanged, pins intact) or a (3,3) matrix = general cells (FAU rhombohedral; fractional round + exact ±1-image search — round-only is inexact at α=60°; parity vs `ase.geometry.get_distances` pinned). **Permutation-invariant structure comparison (2026-09-10):** `species_match_max_disp` (largest MIC atom displacement under the best per-species matching — 0 for a pure relabeling of like atoms) and `same_structure(..., tol=DEDUPE_TOL_ANG)`, the EXACT tolerance test (bipartite perfect matching on the within-tol graph, not the min-sum matching's worst atom). `DEDUPE_TOL_ANG` = 0.75 Å is now the single home of the number `cation.seed_cation_sets` already defaulted to. NOT symmetry-aware by design: two placements related by a space-group operation of the host are energetically degenerate but stay distinct. First consumer: `stage1a_v2/mace_rank.py`'s carry-k dedup. **`group_same_structures(structs, cell, tol=)`** (2026-09-22) partitions a list of structures into same-placement groups (greedy first-fit against each group's first member, input order = priority; `same_structure` is not transitive, so a group is "within tol of its first member") — Foundations f2 re-seed | `tests/na_placement_multicomp/common.py`, `binding/run_range_all.py`, `pipeline_archive/stage1a_v1/mor_core.py`; general-cell path new (Foundations) |
| `framework.py` | Baseline registry (cell read from embedded Lattice + cross-checked → enforces the "update xyz + constant together" rule; **per-baseline `BASELINE_SYMPREC`** — 0.30 is MOR-tuned, not universal), `load_framework` (T-orbits, Löwenstein graph), `loewenstein_ok`, Al samplers, space-group op helpers (`map_atom_under_op` residual FIXED 2026-07-07 — the test-lineage copy dropped the translation, spuriously rejecting t≠0 ops). **Stage-1a v2 enumeration layer (2026-07-07):** `site_permutations`, `canonical_arrangement` (canonical form + exact degeneracy g), `enumerate_al_arrangements` (exhaustive symmetry-distinct Löwenstein classes; MOR n_al=3/4/6 → 844/7,136/173,833), `sample_al_arrangements`, `arrangement_invariants` (stratification/Dedeček axes; single-orbit-safe for FAU). **Counting layer (2026-09-01):** `count_al_arrangements` — the EXACT census of the whole Al-placement space for every n_al at once, without enumerating: Löwenstein-valid counts from the independence polynomial of the Al–O–Al graph, symmetry-distinct counts by Burnside over `site_permutations`, plus `n_al_max` (the framework's Löwenstein Si:Al floor — MOR 16 Al / Si:Al 2.0 because its 5-rings make the graph non-bipartite; FAU 24 Al / Si:Al 1.0). Seconds for the full MOR ladder vs 45.8 s to enumerate Si7 alone; selftest-pinned against the enumerator. Canonical numbers table: `MOR/STAGE1A_INVESTIGATION.md` §1. **Foundations layer (2026-07-10):** general (3,3) cells (loader raise dropped, `fw['cellmat']` always set, MOR (3,)-cell behaviour unchanged), `load_structure` (light loader for relaxed decorated structures — no orbits/4-coordination assert), `t_rings` (no-shortcut T-ring enumeration; FAU 16 6-rings / 36 4-rings pinned), `idealized_primitive` (spglib structure repair — the frozen 500-Ry `Fau-All_Si.xyz` is P1-broken, Fd-3m only at symprec ~1.0; every new FAU calc seeds from its idealized form) | `tests/na_placement_multicomp/common.py`, `pipeline_archive/stage1a_v1/mor_core.py`; enumeration new (design memory `project_stage1a_v2_design`); Foundations layer new |
| `cation.py` | Extra-framework cation model absorbed from `mor_core.py` at the stage-1a v2 rework (the absorption point named below): UFF-LJ + reduced-charge-Coulomb `cation_energy_grad` (ENERGY parity with mor_core pinned at 1e-9; Coulomb-gradient factor-1/2 bug FIXED; as-run eV/kcal units quirk kept deliberately — see docstrings), per-axis image expansion (`images="exact"` wrap-invariant default / `"legacy"` = mor_core ±1 parity; general (3,3) cells use explicit lattice images over `perp_widths`, legacy rejected), `relax_cations`, `place_cations_near_al`, `seed_cation_sets` (deterministic multi-seed Na layer, Hausdorff dedupe; **`n_draw=` UFF-prescreened pools, 2026-09-03** — seeds in UFF-energy order from a fixed pool, extension-stable; the Si11 round-1 protocol is UFF-best 2 of 128), `assemble_atoms` (cations-last convention), **`exchange_na`** (Foundations f2 baseline exchange: Ag/Cu 1:1 at Na positions, Pb/Bi deterministic greedy MIC grouping → centroid + UFF relax; a pre-DFT nudge, NOT a siting search — stage1b_v1 stays DO-NOT-RESURRECT) | `pipeline_archive/stage1a_v1/mor_core.py` (2026-07-07); mor_core keeps its copy as v0/v1 provenance; exchange new (Foundations 2026-07-10) |
| `cp2k.py` | Input builders: `cell_opt_input` / `geo_opt_input` / `energy_force_input` (+`dft_section`, `subsys_section`, `kind_blocks`); RKS defaults byte-identical to the staged multicomp production inputs; UKS/multiplicity for guests; `angles=`/`symmetry=` cell kwargs (FAU `60/60/60` + `RHOMBOHEDRAL`, the validated FAU-era form) and `geo_opt_input(fixed_atoms=(first,last))` → the v0/FAU `&FIXED_ATOMS LIST a..b` frozen-framework screen (contiguous range only — cations-then-guest-last assembly makes every freeze a prefix); `wfn_restart=` → SCF_GUESS RESTART + WFN_RESTART_FILE_NAME (the v0 screen→full-opt wavefunction hand-off); `scf_outer=(inner, outer)` → inner MAX_SCF + &OUTER_SCF loop at unchanged EPS_SCF (the SCF-robustness rescue variant; default renders byte-identically); **`cell_opt_input(pressure_tolerance=bar)`** (2026-09-17) → `PRESSURE_TOLERANCE` inside `&CELL_OPT`, **off by default** so every production input keeps its byte-parity — added for `MOR/tests/cell_treatment_si11`, which measured repeat CELL_OPT of ONE Na basin scattering by mean 6.9 / max 33.1 kJ/mol while every run reports CONVERGED (CP2K's default accepts a 200-bar-wide pressure window); the tightened arm separates "tolerance too loose" from "stress numerically noisy"; **`dft_section(mulliken=True)`** (2026-09-23, reaches every builder via `**dft_kw`) → a `&PRINT &MULLIKEN ON` block so GEO_OPT/CELL_OPT outputs (PRINT_LEVEL LOW) carry per-atom charge/spin for `read_mulliken_spin`; off by default, byte-identical — `Foundations/dissociation_probe/na_control`; `rescue_scf_text(inp_text)` → the same rescue applied POST-HOC to an already-rendered input (byte-parity with the builders pinned; refuses double application) — for cluster-side supervisors that can't reconstruct builder args. `strip_wfn_restart(inp_text)` → the INVERSE post-hoc edit (drop WFN_RESTART_FILE_NAME, SCF_GUESS back to ATOMIC; byte-parity with `wfn_restart=None` pinned, raises when there is nothing to strip) — for a full-opt whose screen wavefunction is not travelling with it (Foundations Perlmutter migration 2026-08-25). Parsers: `final_energy_ha`, `program_ended_ok`, `opt_completed`, **`opt_exit(out)`** (2026-09-24) → HOW an optimisation stopped (`criteria` = standard banner / `lbfgs` = L-BFGS's own energy-flat stop / None) + the last `OPT|` step's values and which criteria were still unmet — `opt_completed` counts both exits as converged, and every FAU f2 cell-opt of the Foundations campaign (17/17) took the `lbfgs` exit with gradient 6–63× its limit and residual pressure up to 1,410 bar, while all 16 MOR took `criteria`; `run_time_seconds` (wall-clock duration of a COMPLETED run — CP2K's own T I M I N G row, banner-timestamp fallback; **None** for a killed/running job, never a partial time, so walltime budgets are never biased low), `read_forces_au`, `read_stress_ase_ev_ang3` (CP2K→ASE **sign flip**), `read_last_cell_abc`, `read_last_cell_vectors` (full 3×3, rhombohedral cell-opts), **`read_input_cell(inp)`** (2026-09-22) → the cell an INPUT declares (the `&SUBSYS` cell — a CELL_OPT input also has a MOTION print `&CELL` key; ABC + ALPHA_BETA_GAMMA → `cellpar_to_cell` matrix, the orientation CP2K builds; raises when ABC is absent) + **`opt_frame_cells(inp, n_frames, cell_file=, restart_cell=)`** → the cell of EVERY trajectory frame (cell-opt: frame k at `.cell` row k plus CP2K's final re-print at the last row, frames = rows + 1 enforced — the trajectory never holds the starting geometry; fixed-cell: the input cell, except a `RESTART_CELL` from another job — the f2 geo-opt, whose input ABC is still the cell-opt's STARTING cell — which raises unless `restart_cell` is passed) — fixed-cell jobs print no `.cell` file, so this is how their final geometries get written out with the cell that actually ran (Foundations advisor geometry export), **`scf_iterations` + `scf_converged`** (2026-09-23) — the last OT SCF cycle as (step, gradient, energy) rows and CP2K's own converged/NOT verdict, for judging an UNCONVERGED single point (energy flat while only the gradient criterion failed = a flagged, usable value; still drifting = not usable) — the isolated Bi atom of `Foundations/atom_references` was the case, **`read_abs_spin_density` + `read_mulliken_spin`** (2026-09-17) — the spin half of a UKS output: the integrated |rho_a − rho_b| and the per-atom Mulliken charge/spin of the LAST analysis block, both **None** on an RKS run rather than a zero nobody computed. They exist because an all-UKS rescore has to tell a GENUINE broken-symmetry solution from a cold-start SCF that merely landed on a worse one — on a multiplicity-1 term E_UKS ≤ E_RKS makes a positive dE provably an instrument fault, and spin settles which it is (Foundations f6; pinned on one closed-shell and one Cu→Cl2 charge-transfer frame). **Job-dir hygiene (2026-09-03):** `clear_generated` (drop CP2K products whenever a job's inputs are rewritten) + `output_is_current` (an output older than the inputs beside it is stale, by timestamp — geometry comparison is invalid) — the two halves of the guard against the 2026-08-17 f3 incident where 245 dirs got new inputs and kept old outputs; any re-runnable package generator must use both | `tests/na_placement_multicomp/setup_dft.py`, `pipeline_archive/stage1a_v1/*-example.inp`, `tests/na_training_set/collect_train.py`; constraints/angles Foundations 2026-07-10 (from `oldbinding/setup_all.py` + `FAU/.../opt1.inp`) |
| `slurm.py` | **Cluster identity (login account + scratch root) is CONFIGURED, never hardcoded** — `resolve_host`/`resolve_remote_base` read the profile attr, then `$ZEOLIB_<PROFILE>_HOST`/`_BASE`, then `~/.config/zeolib/clusters.json`, then RAISE (rule 7: a silently wrong host ships your package elsewhere). The repo is public; everything else in a profile is a public protocol convention. Cluster-profile-aware `sbatch_text` / `cp2k_run_line` / **`gpu_sbatch_text`** (2026-09-24: one-GPU Pronghorn python/MLIP job — billed renter header, conda env from the maceenv registry, fail-loud nvidia-smi + `torch.cuda` gate; extracted from `stage1a_v2/s2_package.py`'s shipped GPU route, first consumer `MOR/tests/mace_volume_states_si11`) / `submit_script_text` / `ship_script_text` + `copy_back_script_text` (the per-package `ship.sh` pusher + `copy_back.sh` puller Marcus runs locally — ship BOTH with EVERY package; **environment-adaptive**: rsync when present, else tar-over-ssh, because Windows Git Bash has ssh+tar but no rsync); **`COPYBACK_EXCLUDES` bars `*.wfn`/`*.restart`/`*.Hessian`** — all regenerable optimizer state, all barred from Drive by the 2026-07-01 cleanup, and Hessians alone were half the 1.7 GB Foundations staging package that timed out on 2026-09-02; the rsync branch is `--partial --timeout=300` + ssh keepalive so a dropped link resumes rather than restarting the in-flight file; **flags must stay openrsync-safe** — macOS 15 ships openrsync (protocol 29), where `--info=...` does not exist and exits 1 with a usage dump. **PRONGHORN** (default: long/high-priority; starts immediately, 14-day cap; container **mpirun, never srun**; OMP=1; partition.sh `psub` s1-first + 50-cap throttle; timestamped lines). **PERLMUTTER** (volume of short jobs, e.g. 300+ SP DFTs: hundreds queue fine, fast nodes, but 1–2 day queue latency + 2-day cap, enforced; shifter `docker:cp2k/cp2k:2022.1` + srun, no `--account`, no `--time-min` (CP2K reads it as an effective walltime and quits after the first SCF energy — 2026-07-14 incident), no throttle; `sbatch_text(qos=)` default `"regular"`, `"debug"` for a fast ~immediate ≤30-min smoke job; **`array_manifest_text` + `array_sbatch_text`** — bundle hundreds of homogeneous jobs into ONE job array (manifest line i ↔ task i, cd + run in that dir; smoke via `sbatch --array=1`, rerun failures via `--array=i,j`); Perlmutter-only — Pronghorn raises, since an array's uniform directives cannot express psub's s1-free-first/s2-overflow routing or the queue-depth throttle). **`pack_groups` + `packed_array_manifest_text` + `packed_array_sbatch_text`** (2026-08-25) are the NODE-HOUR-efficient variant for a metered allocation: each array task packs N CP2K jobs onto ONE node as `--exact` srun steps of 32 ranks each, because CP2K runs at ~34% parallel efficiency from 32→128 ranks on our cells (728.8 s vs 540.1 s, `tests/cp2k_image_parity`), so 4-packed costs 2.9× fewer node-hours than 4×128-rank; a task's wall clock is its SLOWEST member, so pack like with like, and any member's failure fails the task. ⚠ Perlmutter image = CP2K **2022.1**, not the validated 2026 container — never pool its absolute energies with Pronghorn-2026 rankings without cross-image validation | `tests/na_placement_multicomp/setup_dft.py`, `MOR/partition.sh`, global CLAUDE.md; Perlmutter: `tests/na_placement_3al/setup_perlmutter_wsp.py` (369 shipped sbatch files = byte-parity fixtures) |
| `fileio.py` | `write_lf`, `has_crlf`, `write_coords_inc`/`read_coords_inc`; **input-view XYZ (2026-09-22, rule 9)** — `write_coords_inc` also writes `view_xyz_path(path)` (`coords.inc` → `coords.xyz`), a 1-frame extended-XYZ with `Lattice=` from `cell=` ((3,) or (3,3)) or the Atoms' cell; `view_xyz=False` opts out; name can't collide with CP2K's `*-pos-1.xyz`, so `clear_generated` keeps it; **`write_csv_lf`** (2026-08-25) — `csv`'s excel dialect terminates lines with CRLF and that wins over the handle's `newline=`, so the natural `open(p,'w',newline='\n')` + `DictWriter` still writes CRLF; every Foundations result CSV predating this has CRLF endings; **XYZ frames (Foundations 2026-08-17)** — `read_xyz_frames(path, last_only=)` (multi-frame CP2K `*-pos-1.xyz` trajectories; `last_only` = the converged geometry; a truncated trailing frame RAISES per rule 7, since that is what a walltime-killed job leaves) and `write_extxyz(path, frames, lattice=)` (one multi-frame extended-XYZ that OVITO/ASE/VMD open as a flipbook; free text goes into a quoted `info="..."` key so CP2K comment lines can't break key=value parsing) | CRLF working-agreement memory; format from all setup scripts; XYZ frames new (Foundations `communication/`) |
| `molecules.py` | Canonical home of the 7 guest templates (`GUESTS`, `MOL_RADIUS`, `guest_positions`/`guest_atoms`/`guest_elements`) — iodides I2/HI/CH3I + confounders H2O/Cl2/NO2/NO3, origin-centred idealized seeds; key parity with `constants.MULTIPLICITY` and composition parity with the frozen FAU BindingMolecules pinned. **Adsorption-state classifier (2026-09-03):** `template_bonds` (the chemical bonds of each template, covalent-radius rule, pinned counts) + `adsorption_state(name, guest_pos, cell)` → `molecular`/`dissociated` by re-measuring every template bond minimum-image in the relaxed guest (>1.30× template = broken; intact f3 winners sit at 1.02–1.04, the FAU Ag_11 HI proton-transfer winner at 1.41 — pinned on that real frame). Consumed by `Foundations/f3_guests.py collect` (per-site `state`) and `f4_binding.py` (winner state + best-molecular / best-dissociated E_bind per row) | `MOR/oldbinding/setup_all.py` MOLECULES verbatim (Foundations 2026-07-10) |
| `placement.py` | Hand-template guest placement: `build_void_grid` (0.5 Å fractional grid, MIC clearance), `snap_to_void` (R_PROBE 1.2), `place_guest` (≤300 random orientations vs the 1.6 Å clash floor), `random_rotation`, `min_dist_mol_fw`, `ring_normal_sites` (FAU ± ring-normal anchors, driven by `framework.t_rings` not file row order). **Directed product-state construction (2026-09-03):** `fibonacci_directions` (deterministic sphere directions, no RNG) + `bond_site(host_pos, cell, anchor, distance, exclude=)` → where to put a fragment atom BONDED to a host atom at a given distance, in the direction of maximum clearance; `(None, None)` when the anchor is enclosed. Builds a dissociated product directly (H on a framework O at the measured Brønsted O–H, halide on a cation at the measured M–X) instead of hoping a whole-molecule placement finds it — `Foundations/dissociation_probe/`. Orthorhombic results identical to v0 (a shipped Ag_5 SITE_INFO position is reproduced in selftest); general cells via geometry dispatch | `MOR/oldbinding/setup_all.py` + `test_placement.py` (Foundations 2026-07-10) |
...
| `relax.py` | **Pressure-converged ASE/MLIP relaxation (2026-09-24).** `relax(atoms, cell_mode, fmax, maxstep, pressure_tol_bar, keep_angles=True)` stops only when the real atomic forces < fmax AND every relaxed normal stress < `pressure_tol_bar`; variable cell with no tolerance RAISES. Angles held by default (= CP2K KEEP_ANGLES). Plus `cell_filter`, `max_stress_bar`, `pressure_bar`. Exists because an ASE cell filter at fmax 0.05 accepts ~fmax·N/V of residual stress — ~4.3 kbar on 148-atom Na-MOR — which voided R2/R5 of `MOR/tests/mace_volume_states_si11` and affects every past `mace_rank.py --cell variable` result. ASE imported lazily | new; defect found in `stage1a_v2/mace_rank.py`'s inline `relax` |
...
- 2026-09-24 — `cp2k.opt_exit`: which banner ended an optimisation and where
  ...
  L-BFGS's own criterion at 347 bar / gradient 18x limit. A survey of the
  campaign's 33 f2 cell-opts then showed ALL 17 FAU took that exit with
  criteria unmet (median 194 bar, gradient 13.5x limit) and all 16 MOR the
  standard one. `opt_completed` is unchanged (both exits ARE CP2K's verdict);
  the new function makes the difference reportable. Selftest +5, pinned on
  the real FAU Cu_3 f2 cell-opt.
- 2026-09-24 — new `relax.py`: pressure-converged variable-cell relaxation
  (explicit stress criterion, angles held, no silent loose default). The ASE
  cell-filter fmax criterion alone left 1.3-2.6 kbar in the MACE Si11 volume
  test (tolerance ~4.3 kbar at N=148). Selftest reproduces the defect on an EMT
  Cu fixture (>1 kbar left) and pins the fix (<10 bar, angles exact).
  `mace_rank.py` keeps its inline relax (it shipped; fixed-cell production is
  unaffected) — migrate when next touched.
- 2026-09-24 — `slurm.gpu_sbatch_text` + `GPU_PARTITION/ACCOUNT/QOS`: the
  one-GPU python sbatch, extracted from the Si11 ranking route's shipped
  preamble (selftest pins header, env, GPU gate, Pronghorn-only, walltime cap).
  Driven by `MOR/tests/mace_volume_states_si11` (does MACE hold the DFT
  volume states?). `s2_package.py` keeps its inline copy — it shipped.
- 2026-09-23 — `cp2k.dft_section(mulliken=True)`: opt-in per-atom
  Mulliken print inside &DFT, so a GEO_OPT (PRINT_LEVEL LOW) records WHERE
  its spin sits. Driven by the dissociation probe's I2 wave: 16/16 Cu
  products were broken-symmetry singlets (|spin| ~2.2 e) but no output said
  which atoms carried it, and the Na-form negative control built next is
  read chiefly from that. Default byte-identical (every parity pin intact).
  Selftest +3.
...
- 2026-09-22 — `geometry.group_same_structures`: greedy first-fit
  ...
  cation arrangement found among a combo's guest-relaxed hosts (FAU Bi_3: 35
  hosts -> 14 Bi arrangements; FAU Pb_5: 35 -> 11). Selftest +2.
- 2026-09-22 — input-view XYZ protocol (rule 9): `fileio.write_coords_inc`
  now writes a 1-frame extended-XYZ beside every coords file by default,
  plus `view_xyz_path`. Marcus's request: any HPC job dir should open in a
  viewer without reconstructing its geometry. The coords `.inc` is
  byte-unchanged. Checked that no living code mistakes the new file for a
  trajectory (`s4_harvest` takes only `geo-opt*pos*.xyz`, `cp2k.read_forces_au`
  `*-forces-*.xyz`, `compile_structures` scans phase dirs not job dirs).
  Selftest +8.
- 2026-09-10 — `geometry` gained `species_match_max_disp` / `same_structure`
  (permutation-invariant, MIC structure comparison) and `DEDUPE_TOL_ANG`;
  `cation.seed_cation_sets`'s `dedupe_tol` now defaults to that constant
  (same 0.75 Å, no behaviour change). Driven by
  `MOR/pipeline/stage1a_v2/mace_rank.py`, whose DFT carry-k picked the K
  lowest-energy Na SEEDS with no geometric test: over the Si11 ranking
  (`ship_rank_si11/Si11/topk_geoms.extxyz`, 6,954 arrangements, 7,465
  within-arrangement pairs) 1,687 pairs — 20 % of arrangements — are the same
  placement twice, 330 of them visible only under a permutation-invariant test
  and 143 more only under MIC. The measured distribution is bimodal with 0.75 Å
  in the empty band (1,687 below / 382 between 0.75 and 2.0 / 5,396 above), so
  the shared tolerance is not a close call in either direction. Selftest +10.

- 2026-09-03 — `cation.seed_cation_sets` gained `n_draw=` (UFF-prescreened
  seed pools). Driven by the Stage-1a scale-up review
  (`MOR/STAGE1A_SCALEUP_REVIEW.md` §1.4): with 3 random seeds the ranker's
  round-1 gate drops ~1 good Al arrangement in 10; the seeder's own UFF
  energy ranks a candidate's Na placements at within-candidate Spearman
  +0.65 vs MACE (118 Si15 production candidates), and taking the UFF-best
  2 of the pool cuts the gate loss to ~1 %. Pool is a pure function of
  (rng, n_draw) so seed k is extension-stable; `n_draw=None` is the legacy
  path, verified byte-identical vs the previous commit. First consumer:
  `MOR/pipeline/stage1a_v2/mace_rank.py --n-draw 128` (Si11 GPU package).
  Selftest +4. Commit `722e950`.

- 2026-08-25 — repo made PUBLIC, and cluster login identities removed from
  committed code in the same change. `slurm.py` profiles no longer carry
  a login account or the site work path; `resolve_host` /
  `resolve_remote_base` resolve them from the profile attribute, then env, then
  `~/.config/zeolib/clusters.json`, then raise. Generated `ship.sh`/
  `copy_back.sh` are byte-identical to before once configured. Selftest pins
  the generated scripts against EXAMPLE identities, checks the unconfigured
  path raises, and greps `slurm.py` so a real account cannot creep back in.
  Not a credential exposure (both clusters need key + MFA) but it named two
  real accounts and their work path. Selftest +3.
- 2026-08-26 — constants gained `VALENCE_ELECTRONS` (derived from the
  `GTH-PBE-qN` suffixes, so it cannot drift from the potentials actually used)
  and `valence_electron_count(symbols, charge=0)`. Added for
  `MOR/tests/rks_uks_parity`, which asks whether RKS and UKS give the same
  energy on our closed-shell systems: CP2K's `MULTIPLICITY` defaults to 0 =
  "derive it" (even electron count -> 1, odd -> 2), so `UKS` with no
  MULTIPLICITY line describes the same closed-shell state as RKS **only** for
  an even count, and RKS itself is only defined there. The builder asserts the
  parity of every system instead of learning it from a CP2K abort. Unknown
  element raises (rule 7). Selftest +4.
...
- 2026-09-02 — `COPYBACK_EXCLUDES` gained `*.Hessian` / `*-BFGS.Hessian`,
  and the generated `copy_back.sh` rsync branch gained `--partial`,
  `--timeout=300` and an ssh keepalive. Driven by the Foundations
  Perlmutter pull dying on a broken pipe: 509 Hessians were 850 MB of a
  1.7 GB package and `merge_results.py`'s KEEP allowlist discards every
  one, so half the transfer existed only to be thrown away. Selftest +1
  (215 checks). Note a re-run costs a fresh 2FA prompt, so a long pull
  is better driven over one ssh ControlMaster socket — see
  `Foundations/migration/pull_perlmutter.sh`.
